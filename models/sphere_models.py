...
#---------------------------------------------------
#------------------Fifth attempt : attention + convolution + attention----------------
#---------------------------------------------------
...
class SpherePointNetRingAttentionAndConvolution2(nn.Module):
    def __init__(self, ring=3, input_channels=4):
        super(SpherePointNetRingAttentionAndConvolution2, self).__init__()
        # self.attn_block1 = TransformerEncoderBlock(input_dim=input_channels, embed_dim=64, num_heads=4, ffn_hidden=64, dropout=0.1)
        # self.attn_block2 = TransformerEncoderBlock(input_dim=64, embed_dim=128, num_heads=4, ffn_hidden=256, dropout=0.1)
        # self.conv3 = nn.Conv1d(128, 256, 3, padding='same')
        # self.conv4 = nn.Conv1d(256, 512, 3, padding='same')

        self.conv1 = nn.Conv1d(input_channels, 16, 1)
        self.attn_block2 = TransformerEncoderBlock3(input_dim=16, embed_dim=64, num_heads=12)
        self.attn_blobk2x = TransformerEncoderBlock3(input_dim=16, embed_dim=64, num_heads=12)
        self.attn_block2y = TransformerEncoderBlock3(input_dim=16, embed_dim=64, num_heads=12)
        self.attn_blobk2z = TransformerEncoderBlock3(input_dim=16, embed_dim=64, num_heads=12)
        self.conv3 = nn.Conv1d(16, 64, 1)
        self.attn_block4 = TransformerEncoderBlock3(input_dim=64, embed_dim=256, num_heads=12)
        self.attn_blobk4x = TransformerEncoderBlock3(input_dim=64, embed_dim=256, num_heads=12)
        self.attn_block4y = TransformerEncoderBlock3(input_dim=64, embed_dim=256, num_heads=12)
        self.attn_blobk4z = TransformerEncoderBlock3(input_dim=64, embed_dim=256, num_heads=12)
        self.conv5 = nn.Conv1d(64 + 2, 128, 1)
        self.conv6 = nn.Conv1d(128, 256, 1)
        self.conv7 = nn.Conv1d(256, 512, 1)
        self.conv8 = nn.Conv1d(512, 1024, 1)
    ...
    def forward(self, x, point_xyz, mask):
        ...
        x = x.permute(0, 2, 1)
        x = self.attn_block2(x, attn_mask=mask_for_attn)
        x = self.attn_blobk2x(x, attn_mask=mask_for_attn)
        x = self.attn_block2y(x, attn_mask=mask_for_attn)
        x = self.attn_blobk2z(x, attn_mask=mask_for_attn)
        x = x.permute(0, 2, 1)
        x = self.relu(self.conv3(x))
        x = x.permute(0, 2, 1)
        x = self.attn_block4(x, attn_mask=mask_for_attn)
        x = self.attn_blobk4x(x, attn_mask=mask_for_attn)
        x = self.attn_block4y(x, attn_mask=mask_for_attn)
        x = self.attn_blobk4z(x, attn_mask=mask_for_attn)
        x = x.permute(0, 2, 1)
        x = torch.cat([x, euclid, cos_angle], dim=1)
        x = self.relu(self.conv5(x))
...
#---------------------------------------------------
#------------------Six attempt : attention + convolution + attention----------------
#---------------------------------------------------
...
class SpherePointNetRingAttentionAndConvolution3(nn.Module):
    def __init__(self, ring=3, input_channels=4):
        super(SpherePointNetRingAttentionAndConvolution3, self).__init__()
        # self.attn_block1 = TransformerEncoderBlock(input_dim=input_channels, embed_dim=64, num_heads=4, ffn_hidden=64, dropout=0.1)
        # self.attn_block2 = TransformerEncoderBlock(input_dim=64, embed_dim=128, num_heads=4, ffn_hidden=256, dropout=0.1)
        # self.conv3 = nn.Conv1d(128, 256, 3, padding='same')
        # self.conv4 = nn.Conv1d(256, 512, 3, padding='same')
        ...
        self.conv1 = nn.Conv1d(input_channels, 16, 1)
        self.attn_block2 = TransformerEncoderBlock4(input_dim=16, embed_dim=32)
        self.attn_blobk2x = TransformerEncoderBlock4(input_dim=16, embed_dim=32)
        self.attn_block2y = TransformerEncoderBlock4(input_dim=16, embed_dim=32)
        self.attn_blobk2z = TransformerEncoderBlock4(input_dim=16, embed_dim=32)
        self.conv3 = nn.Conv1d(16, 64, 1)
        self.attn_block4 = TransformerEncoderBlock4(input_dim=64, embed_dim=64)
        self.attn_blobk4x = TransformerEncoderBlock4(input_dim=64, embed_dim=64)
        self.conv5 = nn.Conv1d(64 + 2, 128, 1)
        self.conv6 = nn.Conv1d(128, 256, 1)
        self.conv7 = nn.Conv1d(256, 512, 1)
