...
if __name__ == "__main__":
    ...
    for i, func in enumerate(funcs):
        for h in tqdm(np.arange(0.03, 0.1, 0.01), desc=f"Func {i}", leave=False):
            (high_res_graph, high_res_faces, high_res_points, high_res_mesh, high_res_h),\
                (sparse_graph, sparse_faces, sparse_points, sparse_mesh, sparse_h), mapping\
                = make_uniform_surface_multimesh(f=func, radius=5.0, spacing=h, sparse_stride=5)
            ...
            # distances, predecessors = fmm(csgraph=high_res_graph, directed=False, indices=high_res_source_idx, return_predecessors=True)
            distances, predecessors = fmm(V=high_res_points, F=high_res_faces, source_index=high_res_source_idx)
