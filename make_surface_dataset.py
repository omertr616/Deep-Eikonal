...
surface_functions = {
    ...
    bump_grid: [{'A': A, 'sigma': s, 'spacing': sp} for A in [0.5, 1.0] for s in [0.5, 1.0] for sp in [2, 3]],
}
...
mixed_functions = {
    mix1: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
    mix2: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
    mix3: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
    mix4: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
    mix5: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
    mix6: [{'alpha': a} for a in [0.3, 0.5, 0.7]],
}

# -- Combine everything
param_grid = {**surface_functions, **mixed_functions}
...
for f, param in tqdm(all_configs, desc="Generating surfaces"):
        try:
            ...
            (graph_high_res, faces_high_res, points3d_high_res, mesh_high_res, h_high_res), sub_meshes, coor_to_index = \
                make_uniform_surface_multimesh(f=surface_func, radius=5, spacing=0.005, sparse_strides=[10, 20])
